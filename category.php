    ...
    <div class="row">
      <div class="col-md-8">
        ...
        <div class="post-container">
          ...
          $pageSql = "SELECT * FROM category where categoryId = {$id} ";
        $pageResult = mysqli_query($conn,$pageSql) or die('Query Failed.');
        $pageRow = mysqli_fetch_assoc($pageResult);
           ?>
          <h2 class="page-heading">Category : <?php echo $pageRow['categoryName'] ?></h2>
          <?php 
        ...
        $offset = ($page - 1) * $limit;
        $sql = "SELECT p.postId,p.title,p.description,c.categoryName,p.author,p.category,p.postDate,p.postImg,u.userName from post p
        ...
        if (mysqli_num_rows($pageResult) > 0) {
          $totalRecords = $pageRow['post'];
          $totalPage = ceil($totalRecords/$limit);
          echo "<ul class='pagination'>";
          if ($page > 1) {
            echo '<li><a href="category.php?page='.($page-1).'&id='.$id.'">Prev</a></li>';
          }
          for ($i = 1; $i <= $totalPage; $i++) {
            ...
            echo '<li class='.$active.'><a href="category.php?page='.$i.'&id='.$id.'">'.$i.'</a></li>';
          }
          if ($page < $totalPage) {
            echo '<li><a href="category.php?page='.($page+1).'&id='.$id.'">Next</a></li>';
          }
          echo "</ul>";
        }
        ?>
